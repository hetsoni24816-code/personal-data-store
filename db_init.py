...
def ok_mark() -> str:
    """Return a check mark only if the console supports UTF-8; otherwise [OK]."""
...
def exec_many(conn: sqlite3.Connection, statements: Iterable[str]) -> None:
    """Execute a list of SQL statements in a single transaction, skipping blanks."""
    with conn:  # transactional context; commits on success, rolls back on error
        cur = conn.cursor()
        try:
            for s in statements:
                if s and s.strip():
...
def init_schema() -> None:
    with get_conn() as conn:
        ...
        # Backfill columns for users (if table existed previously)
...
def seed_demo() -> None:
    ...
    with get_conn() as conn, conn:  # transactional
        # user
        conn.execute(
