# pages/0_Login.py
# Login / Sign Up with user/org/admin roles (positional create_user compatibility)
...
def _mk_user(first_name: str, last_name: str, dob_iso: str, email: str, password: str):
    """Create a normal user; handle older create_user signatures (no DOB)."""
    try:
...
def _mk_admin(first_name: str, last_name: str, dob_iso: str, email: str, password: str):
    """Create an admin; handle older create_user signatures."""
    try:
        return create_user(first_name, last_name, dob_iso, email, password, "admin", True)
    except TypeError:
        return create_user(first_name, last_name, "1900-01-01", email, password, "admin", True)
...
with tab_signup:
    ...
    role_choice = st.radio(
        "Account type",
        options=["user", "org", "admin"],
        index=0,
        horizontal=True,
        key="signup_role",
        help="Choose 'org' for organisations. 'admin' requires an invite code (set in app secrets).",
    )
    ...
    if role_choice == "user":
        with st.form("signup_user_form"):
            ...
            pw_new    = st.text_input(
                "Create password (min 8 chars, letters & numbers)",
                type="password", key="su_user_pw"
            )

            st.markdown('<div class="block-btn">', unsafe_allow_html=True)
            submit_user = st.form_submit_button("Sign Up")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    # ----- ADMIN -----
    else:  # role_choice == "admin"
        with st.form("signup_admin_form"):
            c1, c2 = st.columns(2)
            with c1:
                first_name = st.text_input("First name", key="su_admin_first")
            with c2:
                last_name  = st.text_input("Last name",  key="su_admin_last")

            dob = st.date_input("Date of birth", format="YYYY-MM-DD", key="su_admin_dob")
            email_new = st.text_input("Admin email", key="su_admin_email")
            pw_new    = st.text_input(
                "Create password (min 8 chars, letters & numbers)",
                type="password", key="su_admin_pw"
            )
            admin_code = st.text_input(
                "Admin invite code",
                type="password",
                key="su_admin_code",
                help="Ask an existing admin for the invite code.",
            )

            st.markdown('<div class="block-btn">', unsafe_allow_html=True)
            submit_admin = st.form_submit_button("Create Admin Account")
            st.markdown('</div>', unsafe_allow_html=True)

        if submit_admin:
            email_norm = _norm_email(email_new)
            if not (first_name and last_name and isinstance(dob, date) and email_norm and pw_new and admin_code):
                st.error("Please fill all fields and enter the admin invite code.")
            elif dob > date.today():
                st.error("Date of birth cannot be in the future.")
            elif not valid_email(email_norm):
                st.error("Invalid email format.")
                ...
                if not ok_pw:
                    ...
                    if not expected:
                        ...
                        try:
                            ok, msg = _mk_admin(first_name.strip(), last_name.strip(),
                                                dob.isoformat(), email_norm, pw_new)
                            if ok:
